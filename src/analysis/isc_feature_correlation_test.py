...
DEFAULT_FEATURES = ["min_lum", "mean_lum", "max_lum", "diff_lum", "amp_rms", "amp_peak", "ebu_r128_M"]
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--output-csv", type=Path, default=Path("out/isc_feature_correlation_stats.csv"))
    p.add_argument("--output-plot", type=Path, default=Path("out/isc_feature_correlation_stats.png"))
    return p.parse_args()
...
def load_features(feature_csv: Path, window_times: np.ndarray, features: list[str]) -> dict[str, np.ndarray]:
    ...
    for feat in features:
        if feat not in feat_df.columns:
            ...
            continue
        out[feat] = np.interp(window_times, feat_df["timestamp"].values, feat_df[feat].values)
    return out
...
def main() -> None:
    ...
    for ci, comp in enumerate(args.components):
        ...
        for fi, feat in enumerate(args.features):
            ...
            null = stats_utils.circular_shift_null(isc_values, statistic_fn)  # (n_shifts,)
    ...
    q, sig = stats_utils.benjamini_hochberg(df["p_exact"].values, alpha=args.fdr_alpha)
    df["p_fdr"] = q
    df["significant"] = sig
    ...
    # ── Heatmap ───────────────────────────────────────────────────────────
    q_matrix = df["p_fdr"].values.reshape(r_matrix.shape)
    abs_max = np.nanmax(np.abs(r_matrix))
    fig, ax = plt.subplots(figsize=(max(3.5, len(args.features) * 1.8), max(2, len(args.components) * 0.9)))
    ...
    for ci in range(len(args.components)):
        for fi in range(len(args.features)):
            ...
            sig_str = "***" if q_val < 0.001 else "**" if q_val < 0.01 else "*" if q_val < 0.05 else ""
            text_color = "white" if abs(r_matrix[ci, fi]) > abs_max * 0.6 else "black"
            ax.text(fi, ci, f"{r_matrix[ci, fi]:.3f}{sig_str}", ha="center", va="center", fontsize=8, color=text_color)

    ax.set_title(f"Pearson r (FDR-corrected) — ISC × Stimulus Features\n{args.stimulus}", fontsize=10)
